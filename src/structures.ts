...
export const isAnyRecord: Guard<Record<PropertyKey, unknown>> = (
  value
...
export function isArrayOf<T>(guard: Guard<T>): Guard<T[]> {
  if (typeof guard !== "function") {
    throw new TypeError(
      `isArrayOf expects a guard parameter. Got instead: ${JSON.stringify(guard)}`
    );
  }
...
export function isRecordOf<K extends PropertyKey>(
  keyGuard: Guard<K>
...
export function isRecordOf<K extends PropertyKey, V>(
  ...
  if (typeof keyGuard !== "function") {
    throw new TypeError(
      `isRecordOf keyGuard expects a guard parameter. Got instead: ${JSON.stringify(keyGuard)}`
    );
  } else if (valueGuard != null && typeof valueGuard !== "function") {
    throw new TypeError(
      `isRecordOf valueGuard expects an optional guard parameter. Got instead: ${JSON.stringify(valueGuard)}`
    );
  }
...
export function isObjectOf<O extends object>(
  ...
  if (schemaUnknown == null || typeof schemaUnknown !== "object") {
    throw new TypeError(
      `isObjectOf expects a guard schema object. Got instead: ${JSON.stringify(schemaUnknown)}`
    );
  }
